...
const char *const nml_file = "./client.nml";
...
int sendJogStop(int jnum, int jjogmode)
{
	...
	if (((jjogmode == JOGJOINT) && (emcStatus->motion.traj.mode == EMC_TRAJ_MODE::TELEOP)) || ((jjogmode == JOGTELEOP) && (emcStatus->motion.traj.mode != EMC_TRAJ_MODE::TELEOP)))
	{
		return -1;
	}
