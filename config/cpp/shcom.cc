...
int updateError()
{
	...
	case EMC_OPERATOR_ERROR_TYPE:
		strncpy(error_string,
				((EMC_OPERATOR_ERROR *)(emcErrorBuffer->get_address()))->error, LINELEN - 1);
		error_string[NML_ERROR_LEN - 1] = 0;
		break;

	case EMC_OPERATOR_TEXT_TYPE:
		strncpy(operator_text_string,
				((EMC_OPERATOR_TEXT *)(emcErrorBuffer->get_address()))->text, LINELEN - 1);
...
int emcCommandWaitDone()
{
	...
	for (end = 0.0; emcTimeout <= 0.0 || end < emcTimeout; end += EMC_COMMAND_DELAY)
	{
		...
		if (emcStatus->status == RCS_STATUS::RCS_DONE)
		{
			return 0;
		}

		if (emcStatus->status == RCS_STATUS::RCS_ERROR)
		{
			return -1;
		}
...
int sendEstop()
{
	...
	state_msg.state = EMC_TASK_STATE_ENUM::EMC_TASK_STATE_ESTOP;
...
int sendEstopReset()
{
	...
	state_msg.state = EMC_TASK_STATE_ENUM::EMC_TASK_STATE_ESTOP_RESET;
	emcCommandSend(state_msg);
	if (emcWaitType == EMC_WAIT_RECEIVED)
	{
		return emcCommandWaitReceived();
...
int sendMachineOn()
{
	...
	state_msg.state = EMC_TASK_STATE_ENUM::EMC_TASK_STATE_ON;
	emcCommandSend(state_msg);
	if (emcWaitType == EMC_WAIT_RECEIVED)
	{
		return emcCommandWaitReceived();
...
int sendMachineOff()
{
	...
	state_msg.state = EMC_TASK_STATE_ENUM::EMC_TASK_STATE_OFF;
	emcCommandSend(state_msg);
	if (emcWaitType == EMC_WAIT_RECEIVED)
	{
		return emcCommandWaitReceived();
...
int sendMdi()
{
	...
	if (emcWaitType == EMC_WAIT_RECEIVED)
	{
		std::cout << "sendMdi received\n";
		return emcCommandWaitReceived();
	}
	else if (emcWaitType == EMC_WAIT_DONE)
	{
		std::cout << "sendMdi done\n";
		return emcCommandWaitDone();
	}
...
int sendJogStop(int jnum, int jjogmode)
{
	...
	if (((jjogmode == JOGJOINT) && (emcStatus->motion.traj.mode == EMC_TRAJ_MODE_TELEOP)) || ((jjogmode == JOGTELEOP) && (emcStatus->motion.traj.mode != EMC_TRAJ_MODE_TELEOP)))
	{
		return -1;
	}
